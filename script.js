document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------- About page: Meet Our Team scroll-jack.
     Same mechanic as the homepage Services section — vertical scroll
     through a tall spacer drives horizontal movement — but only the
     right-hand card track moves; the left intro panel stays fixed in
     place, and translation is percentage-based since the track isn't
     full-viewport width. Disabled on mobile/reduced-motion, where CSS
     already swaps in a plain stacked-card fallback. ---------- */
  const teamSpacer = document.getElementById('teamHScrollSpacer');
  const teamHScroll = document.getElementById('teamHScroll');
  const teamTrack = document.getElementById('teamTrack');

  if (teamSpacer && teamHScroll && teamTrack) {
    const TEAM_MOBILE_BREAKPOINT = 900;
    const teamSlideCount = teamTrack.children.length;
    const teamDots = Array.from(document.querySelectorAll('#teamDots .our-team__dot'));
    const teamReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    let teamTicking = false;

    const clearTeamStyles = () => {
      teamHScroll.style.position = '';
      teamHScroll.style.top = '';
      teamHScroll.style.left = '';
      teamHScroll.style.width = '';
      teamHScroll.style.height = '';
      teamTrack.style.transform = '';
    };

    const updateTeamDots = (index) => {
      teamDots.forEach((dot, i) => dot.classList.toggle('is-active', i === index));
    };

    const updateTeam = () => {
      teamTicking = false;

      if (window.innerWidth <= TEAM_MOBILE_BREAKPOINT || teamReducedMotion) {
        clearTeamStyles();
        return;
      }

      const spacerRect = teamSpacer.getBoundingClientRect();
      const scrollableDistance = teamSpacer.offsetHeight - window.innerHeight;
      let progress;

      if (spacerRect.top > 0) {
        teamHScroll.style.position = 'absolute';
        teamHScroll.style.top = '0px';
        teamHScroll.style.left = '0px';
        teamHScroll.style.width = '100vw';
        teamHScroll.style.height = '100vh';
        progress = 0;
      } else if (spacerRect.bottom <= window.innerHeight) {
        teamHScroll.style.position = 'absolute';
        teamHScroll.style.top = Math.max(0, teamSpacer.offsetHeight - window.innerHeight) + 'px';
        teamHScroll.style.left = '0px';
        teamHScroll.style.width = '100vw';
        teamHScroll.style.height = '100vh';
        progress = 1;
      } else {
        teamHScroll.style.position = 'fixed';
        teamHScroll.style.top = '0px';
        teamHScroll.style.left = '0px';
        teamHScroll.style.width = '100vw';
        teamHScroll.style.height = '100vh';
      ...
      const translatePercent = progress * (teamSlideCount - 1) * 100;
      teamTrack.style.transform = `translateX(-${translatePercent}%)`;
      updateTeamDots(Math.round(progress * (teamSlideCount - 1)));
    };

    const onTeamScrollOrResize = () => {
      if (!teamTicking) {
        teamTicking = true;
    ...
    window.addEventListener('scroll', onTeamScrollOrResize, { passive: true });
    window.addEventListener('resize', onTeamScrollOrResize);
    updateTeam();
  }
});
